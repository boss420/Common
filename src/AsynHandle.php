...
class AsynHandle {
	...
	//解析URL并创建资源句柄
	private function analyzeUrl($sock_flag = 0) {
		...
		case 'https':
			if ($sock_flag) {
				...
				$this->fop = stream_socket_client('ssl://' . $this->host . ":443", $errno, $errstr, $this->timeout, STREAM_CLIENT_ASYNC_CONNECT | STREAM_CLIENT_CONNECT);
		...
		default:
			if ($sock_flag) {
				...
				$this->fop = stream_socket_client($this->host . ":" . $this->port, $errno, $errstr, $this->timeout, STREAM_CLIENT_ASYNC_CONNECT | STREAM_CLIENT_CONNECT);
			}
			break;
		}
	...
	private function checkRecvHeader($header) {
		if (strstr($header, ' 301 ') || strstr($header, ' 302 ')) {
			//重定向处理
	...
	//请求函数,只请求,不返回
	public function Request($url, $sock_flag = 0, $cookie = array(), $post = array(), $timeout = 3) {
		$this->url = $url;
		$this->cookie = $cookie;
		$this->post = $post;
	...
	//获取函数,请求并返回
	public function Get($url, $sock_flag = 0, $cookie = array(), $post = array(), $timeout = 30) {
		$this->url = $url;
		$this->cookie = $cookie;
		$this->post = $post;
		$this->timeout = $timeout;
